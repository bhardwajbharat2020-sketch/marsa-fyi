-- Complete Database Schema for Marsa FYI Application
-- This schema includes all necessary tables for user management, roles, products, orders, and workflow
...
('captain', 'Workflow captain who oversees transactions'),
('seller', 'Product seller/vendor'),
('buyer', 'Product buyer'),
('surveyor', 'Quality surveyor'),
('arbitrator', 'Dispute arbitrator'),
('insurance', 'Insurance agent'),
('transporter', 'Logistics and transportation provider'),
...
CREATE OR REPLACE FUNCTION generate_vendor_code(role_name VARCHAR)
RETURNS VARCHAR AS $$
DECLARE
    prefix VARCHAR(10);
    year_part VARCHAR(2);
...
BEGIN
    ...
    CASE role_name
        WHEN 'seller' THEN prefix := 'VEND';
        WHEN 'buyer' THEN prefix := 'BUY';
        WHEN 'captain' THEN prefix := 'CAPT';
        WHEN 'admin' THEN prefix := 'ADM';
        WHEN 'hr' THEN prefix := 'HR';
        WHEN 'accountant' THEN prefix := 'ACC';
        WHEN 'arbitrator' THEN prefix := 'ARB';
        WHEN 'surveyor' THEN prefix := 'SUR';
        WHEN 'insurance' THEN prefix := 'INS';
        WHEN 'transporter' THEN prefix := 'TRN';
        WHEN 'logistics' THEN prefix := 'LOG';
        WHEN 'cha' THEN prefix := 'CHA';
        ELSE prefix := 'USER';
    END CASE;
...
BEGIN
    IF NEW.vendor_code IS NULL THEN
        -- This would need to be adjusted to get the user's primary role
        NEW.vendor_code := generate_vendor_code('user');
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- Note: You might want to adjust this to use the user's actual role
...
-- ('john_seller', 'john@example.com', '+1234567890', 'John', 'Seller', 'hashed_password_here', 'VEND-23-ABC123'),
